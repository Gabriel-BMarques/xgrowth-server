...
const BriefMarket = require('../model/briefMarket.model');
...
exports.create = async (req, res, next) => {
  try {
    const entity = new BriefMarket(req.body);
    if (req.user) {
...
exports.get = async (req, res, next) => {
  try {
    const entity = await BriefMarket.findById(req.params.id);
...
exports.listByBriefId = async (req, res, next) => {
  try {
    let entities = await BriefMarket.find({ BriefId: mongoose.Types.ObjectId(req.params.id) })
      .populate('CountryId');
...
exports.list = async (req, res, next) => {
  try {
    ...
    let entities = await BriefMarket.find(query);
...
/**
 * Update an existing BriefMarket
 * @public
 */
exports.update = async (req, res, next) => {
  try {
    ...
    const oldEntity = await BriefMarket.findById(entity._id);
    ...
    const savedEntity = await BriefMarket.findById(entity._id);
